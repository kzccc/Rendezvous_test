...
parser.add_argument('--data_dir', type=str,
                    default='/workspace/CHEN_ZK/Rendezvous/rendezvous/pytorch/CholecT45/CholecT45 - 副本/',
...
parser.add_argument('--dataset_variant', type=str, default='cholect45-crossval',
                    choices=['cholect50', 'cholect45', 'cholect50-challenge', 'cholect50-crossval',
                             'cholect45-crossval'], help='Variant of the dataset to use')
...
def train_loop(dataloader, model, activation, loss_fn_i, loss_fn_v, loss_fn_t, loss_fn_ivt, optimizers, scheduler,
               epoch):
    start = time.time()
    ...
    for sch in scheduler:
        sch.step()
    print(
        f'completed | Losses => i: [{loss_i.item():.4f}] v: [{loss_v.item():.4f}] t: [{loss_t.item():.4f}] ivt: [{loss_ivt.item():.4f}] >> eta: {(time.time() - start):.2f} secs',
        file=open(logfile, 'a+'))


def atest_loop(dataloader, model, activation, final_eval=False):
...
val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=True, prefetch_factor=3 * batch_size,
                            num_workers=3, pin_memory=True, persistent_workers=True, drop_last=False)
...
module_i = list(
    set(model.parameters()) - set(model.encoder.cagam.parameters()) - set(model.encoder.bottleneck.parameters()) - set(
...
scheduler_i = torch.optim.lr_scheduler.SequentialLR(optimizer_i, schedulers=[scheduler_ia, scheduler_ib],
                                                    milestones=[warmups[0] + 1])
...
scheduler_vt = torch.optim.lr_scheduler.SequentialLR(optimizer_vt, schedulers=[scheduler_vta, scheduler_vtb],
                                                     milestones=[warmups[1] + 1])
...
if is_train:
    for epoch in range(0, epochs):
        try:
            # Train
            print("Traning | lr: {} | epoch {}".format([lr.get_last_lr() for lr in lr_schedulers], epoch), end=" | ",
                  file=open(logfile, 'a+'))
            train_loop(train_dataloader, model, activation, loss_fn_i, loss_fn_v, loss_fn_t, loss_fn_ivt, optimizers,
                       lr_schedulers, epoch)
            ...
            if epoch % val_interval == 0:
                ...
                atest_loop(val_dataloader, model, activation, final_eval=False)
...
if is_test:
    ...
    for test_dataloader in test_dataloaders:
        atest_loop(test_dataloader, model, activation, final_eval=True)
    if set_chlg_eval:
